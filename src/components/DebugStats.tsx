...
const DebugStats = () => {
  ...
  useEffect(() => {
    const updateCallback = () => {
      ...
      setPosition(`${position.x.toFixed(2)}, ${position.y.toFixed(2)}, ${position.z.toFixed(2)}`)
      setRotation(`${radToDeg(rotation.x).toFixed(2)}, ${radToDeg(rotation.y).toFixed(2)}, ${radToDeg(rotation.z).toFixed(2)}`)
      setScale(`${scale.x.toFixed(2)}, ${scale.y.toFixed(2)}, ${scale.z.toFixed(2)}`)
